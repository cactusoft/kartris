...
Partial Class _Checkout
        ...
        If Not Page.IsPostBack Then
            ...
            Try
                ...
                For Each strGatewayEntry As String In arrPaymentsMethods
                    ...
                    'We shoud have 4 bits of data (in a zero
                    'based array).
                    '---------------------------------------
        ...
        If Not IsNothing(CurrentLoggedUser) Then
            ...
            If Not Page.IsPostBack Then
                ...
                If Not CurrentLoggedUser.DefaultBillingAddressID = CurrentLoggedUser.DefaultShippingAddressID Then
                    ...
                    'Uncheck the 'same shipping/billing address' checkbox
                ...
                Else
                    ...
                    'Check the 'same shipping/billing address' checkbox
    ...
    Private Sub RefreshShippingMethods(Optional ByVal strControl As String = "shipping")
        ...
        '=======================================
        'DETERMINE IF EU VAT NUMBER FIELD SHOWS
        'Need this if:
        '
        '1. EU VAT country of store set in 
        'config settings
        '
        '2. User is in EU country other than the
        'one where store is
        '=======================================
        If Not String.IsNullOrEmpty(GetKartConfig("general.tax.euvatcountry")) Then
            ...
            If adrShipping IsNot Nothing Then
                'Country of user not same as store
            ...
            End If
        Else
            'No EU VAT country specified for the 
            'store in config settings, i.e. VAT
            'support turned off
            phdEUVAT.Visible = False
        End If
    ...
    Protected Sub btnProceed_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnProceed.Click
        If String.IsNullOrEmpty(_SelectedPaymentMethod) Then _SelectedPaymentMethod = ddlPaymentGateways.SelectedValue
        ...
        If mvwCheckout.ActiveViewIndex = "1" Then
            ...
            If Page.IsValid And (UC_BasketView.SelectedShippingID <> 0 Or UC_BasketView.GetBasket.AllFreeShipping) And blnValidEUVAT Then
                ...
                If LCase(clsPlugin.GatewayType) = "local" And Not clsPlugin.GatewayName.ToLower = "po_offlinepayment" Then
        ...
        ElseIf mvwCheckout.ActiveViewIndex = "2" Then
            ...
            If LCase(clsPlugin.GatewayType) <> "local" Or blnValid Or clsPlugin.GatewayName.ToLower = "po_offlinepayment" Then
                ...
                'Concatenate billing address; although we store individual
                'addresses as separate fields, we also store the actual
                'address of an order with the order as a single entity. This
                'way, if the user changes or deletes the stored address later,
                'the order still shows the address they gave at checkout.
                ...
                'Similar to above, for shipping address
                If chkSameShippingAsBilling.Checked Then
                ...
                With UC_BillingAddress.SelectedAddress
                    sbdBodyText.Append(" " & GetGlobalResourceObject("Email", "EmailText_PurchaseContactDetails") & vbCrLf & _
                         ...
                         " " & GetGlobalResourceObject("Kartris", "FormLabel_EmailAddress") & ": " & UC_KartrisLogin.UserEmailAddress & vbCrLf & _
