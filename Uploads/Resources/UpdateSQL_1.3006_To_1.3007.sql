...
-- From 1.3006 to 1.3007
-- =============================================

/****** Object:  StoredProcedure [dbo].[spKartrisDB_Search]    Script Date: 09/12/2010 14:29:30 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
EXEC dbo.sp_executesql @statement = N'ALTER PROCEDURE [dbo].[spKartrisDB_Search]
(	
...
SET NOCOUNT ON;
	...
	IF @Method = ''exact'' BEGIN	
		...
		EXECUTE(''
		INSERT INTO tblKartrisSearchHelper
		SELECT     @@SPID, 1, NULL, V_ID, NULL, V_ProductID, V_Price, 0, '''''' + @SearchText + ''''''
	...
	END ELSE BEGIN
		...
		EXECUTE(''
		INSERT INTO tblKartrisSearchHelper
		SELECT     @@SPID, 1, NULL, V_ID, NULL, V_ProductID, V_Price, 0, '''''' + @KeyWord + ''''''
		FROM         tblKartrisVersions
...
BEGIN
	...
	IF @Method = ''exact'' BEGIN
		...
		EXECUTE(''
		INSERT INTO tblKartrisSearchHelper
		SELECT     @@SPID, 1, NULL, V_ID, NULL, V_ProductID, V_Price, 0, '''''' + @ExactCriteriaNoNoise + ''''''
		FROM         tblKartrisVersions
		WHERE     (V_Live = 1) AND (V_CodeNumber LIKE '''''' + @SearchText + ''%'''')'' );	
	END ELSE BEGIN
		-- Loop through out the list of keywords and search each keyword
...
END
GO
/****** Object:  StoredProcedure [dbo].[_spKartrisDB_SetupFTS]    Script Date: 09/12/2010 14:29:30 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
/****** Object:  StoredProcedure [dbo].[_spKartrisProductCategoryLink_ChangeSortValue]    Script Date: 01/11/2012 12:26:12 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[_spKartrisProductCategoryLink_ChangeSortValue]
(
	@ProductID as int,
	@CatID as int,
	@Direction as char
...
/****** Object:  StoredProcedure [dbo].[_spKartrisCategoryHierarchy_ChangeSortValue]    Script Date: 01/11/2012 13:51:00 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[_spKartrisCategoryHierarchy_ChangeSortValue]
(
	@ParentID as int,
	@CatID as int,
	@Direction as char
